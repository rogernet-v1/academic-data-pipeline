...
```mermaid
graph TD
    A[arXiv API / Data Sources] <--> B[Spiders & Parsers<br/>httpx / asyncio]
    C[Request Signer<br/>JS Reverse / PyExecJS] <--> B
    B --> D[Redis Deduplicator<br/>Set/Bloom Incremental Filter]
    D --> E[SQLAlchemy 2.0 ORM<br/>Async Engine & Upsert]
    E --> F[(Relational DB<br/>SQLite / PostgreSQL)]
````
